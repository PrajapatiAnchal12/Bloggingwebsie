...
const ManagePosts = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this post?')) {
            try {
                await axios.delete(`https://blog-backend-i5u0.onrender.com/api/posts/${id}`);
                fetchPosts();
    ...
    const handleBulkDelete = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedPosts.length} posts?`)) {
            try {
                await axios.post((import.meta.env.VITE_API_URL || 'https://blog-backend-i5u0.onrender.com') + '/api/posts/bulk-delete', { ids: selectedPosts });
                setSelectedPosts([]);
                fetchPosts();
