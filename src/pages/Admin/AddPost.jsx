...
const AddPost = () => {
    ...
    const handleAddSectionModal = async (e) => {
        e.preventDefault();
        try {
            await axios.post((import.meta.env.VITE_API_URL || 'https://blog-backend-i5u0.onrender.com') + '/api/sections', { name: newSecName });
            setNewSecName('');
            await fetchSections();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                if (id) {
                    const postRes = await axios.get(`https://blog-backend-i5u0.onrender.com/api/posts/${id}`);
    ...
    const handleAddCategory = async (e) => {
        e.preventDefault();
        try {
            await axios.post((import.meta.env.VITE_API_URL || 'https://blog-backend-i5u0.onrender.com') + '/api/categories', { name: newCatName, color: newCatColor });
            setNewCatName('');
            await fetchCategories();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (id) {
                await axios.put(`https://blog-backend-i5u0.onrender.com/api/posts/${id}`, postData);
                navigate('/admin/manage-posts');
            } else {
                await axios.post((import.meta.env.VITE_API_URL || 'https://blog-backend-i5u0.onrender.com') + '/api/posts', postData);
                setTitle('');
                setCategory('');
                setSection('');
                setAuthor('');
    ...
    const handleFileUpload = async (e) => {
        ...
        setUploading(true);
        try {
            const res = await axios.post((import.meta.env.VITE_API_URL || 'https://blog-backend-i5u0.onrender.com') + '/api/upload', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            setImage(`https://blog-backend-i5u0.onrender.com${res.data.image}`);
        } catch (error) {
